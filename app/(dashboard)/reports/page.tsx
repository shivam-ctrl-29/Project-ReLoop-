...
const impactData = [
  { month: 'Jan', co2: 1.2, water: 4200, revenue: 28000 },
  { month: 'Feb', co2: 1.5, water: 5100, revenue: 34000 },
  { month: 'Mar', co2: 1.3, water: 4800, revenue: 31000 },
  { month: 'Apr', co2: 1.8, water: 6200, revenue: 42000 },
  { month: 'May', co2: 2.1, water: 8100, revenue: 56500 },
  { month: 'Jun', co2: 2.4, water: 9200, revenue: 42800 },
];

const depts = [
  { dept: 'Mechanical Dept.', oil: 340, ewaste: 18 },
  { dept: 'Hostel Mess',      oil: 280, ewaste: 4  },
  { dept: 'Computer Lab',     oil: 10,  ewaste: 26 },
  { dept: 'Admin Block',      oil: 80,  ewaste: 12 },
  { dept: 'Sports Complex',   oil: 45,  ewaste: 3  },
];
  ...
  return (
    <div>
      ...
      <div className="flex gap-4 mb-6">
        <StatCard label="CO2 Saved (YTD)" value="10.3" unit="T" change="+2.4T MoM" changeType="up" icon={Leaf} color="green" />
        <StatCard label="Water Conserved (YTD)" value="37.6" unit="K L" change="+9.2K MoM" changeType="up" icon={Droplets} color="blue" />
        <StatCard label="Total Revenue (YTD)" value="₹2,34,300" change="+22% vs target" changeType="up" icon={BarChart2} color="amber" />
        <StatCard label="Items Diverted from Landfill" value="245" change="All time" changeType="info" icon={Recycle} color="green" />
      </div>

      <div className="flex gap-5 mb-6">
        {/* Revenue chart */}
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <h3 className="font-bold text-base mb-4" style={{ color: '#1F2A24' }}>REVENUE & IMPACT — 2026</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={impactData} barCategoryGap="25%" barGap={4}>
              ...
              <Tooltip formatter={(v: number) => [`₹${v.toLocaleString()}`, 'Revenue']} contentStyle={{ borderRadius: 8, border: '1px solid #E5E7EB' }} />
      ...
      <div className="flex gap-5 mb-6">
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="space-y-4">
            {depts.map((d, i) => {
              const total = d.oil + d.ewaste * 10;
              const max = 580;
              const pct = Math.round((total / max) * 100);
              return (
                <div key={i}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium" style={{ color: '#1F2A24' }}>{d.dept}</span>
                    <span className="text-xs" style={{ color: '#5B6B63' }}>{d.oil}L oil · {d.ewaste} items</span>
                  </div>
