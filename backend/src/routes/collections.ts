...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [history, stats, chart] = await Promise.all([
      client.query(`SELECT id, pickup_type, quantity, scheduled_date, status, revenue, buyer_name
                    FROM pickups WHERE user_id=$1 AND status='collected' ORDER BY scheduled_date DESC`, [userId]),
      client.query(`SELECT COUNT(*) AS total,
                    ...
                    FROM pickups WHERE user_id=$1 AND status='collected'`, [userId]),
      client.query(`SELECT TO_CHAR(recorded_month,'Mon') AS month, revenue_oil AS oil, revenue_ewaste AS ewaste
                    FROM impact_records WHERE institution_id=$1 ORDER BY recorded_month ASC`, [institutionId]),
    ]);
    res.json({ history: history.rows, stats: stats.rows[0], chart: chart.rows });
