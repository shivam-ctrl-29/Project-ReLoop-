...
export default function CollectionsPage() {
  ...
  useEffect(() => {
    fetch('/api/collections').then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={chart} barCategoryGap="25%" barGap={4}>
            ...
            <Tooltip formatter={(v: any, name: any) => [`₹${Number(v).toLocaleString('en-IN')}`, name === 'oil' ? 'Used Oil' : 'E-Waste']} contentStyle={{ borderRadius: 8, border: '1px solid #E5E7EB' }} />
