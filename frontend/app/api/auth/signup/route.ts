...
export async function POST(req: NextRequest) {
  const { name, email, password, role, institution, department, phone, docType, docName } = await req.json();
  ...
  try {
    ...
    // Handle Compliance Document
    if (role === 'buyer' && docType && docName) {
      await client.query(
        `INSERT INTO compliance_docs (doc_code, user_id, doc_type, doc_name, issued_date, status, file_url)
         VALUES ($1, $2, $3, $4, NOW(), 'valid', $5)`,
